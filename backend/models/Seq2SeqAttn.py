...
#     # Implementation of RNN decoder for the sequence-to-sequence model.
#     # Function body as provided in the original snippet.
#
#     # Omitted for brevity..
#
...
#         """Initialize all hyperparameters here"""
#         # set hyperparamter
...
def rnn_decoder_with_attention(decoder_inputs, initial_state, cell, loop_function, attention_states, scope=None):
    with tf.variable_scope(scope or "rnn_decoder"):
        ...
        for i, inp in enumerate(decoder_inputs):
            ...
            # Compute possibility of attention
            attention_logits = tf.reshape(attention_logits, shape=(-1, hidden_size))
            V_a = tf.get_variable("V_a", shape=[hidden_size, 1],
                                  initializer=tf.random_normal_initializer(stddev=0.1))
            attention_logits = tf.matmul(attention_logits, V_a)
            ...
            # Compute weighted sum of hidden state for each encoder input as attention state
            p_attention = tf.expand_dims(p_attention, axis=2)
            attention_final = tf.multiply(attention_states_original, p_attention)
            context_vector = tf.reduce_sum(attention_final, axis=1)
...
class Seq2SeqAttn:
    ...
    def inference(self):
        ...
        return tf.reshape(logits, shape=(-1, self.decoder_sent_length, self.vocab_size))

    def loss_seq2seq(self):
        with tf.name_scope("loss_seq2seq"):
            losses = tf.contrib.seq2seq.sequence_loss(logits=self.logits,
                                                      targets=self.input_y,
                                                      weights=tf.ones([self.batch_size, self.decoder_sent_length],
                                                                      dtype=tf.float32),
    ...
    def instantiate_weights(self):
        with tf.name_scope("projection"):
            ...
            self.W_initial_state = tf.get_variable("W_initial_state", shape=[self.hidden_size, self.hidden_size],
                                                   initializer=self.initializer)
            self.b_initial_state = tf.get_variable("b_initial_state", shape=[self.hidden_size])

            self.gru_cell = tf.nn.rnn_cell.GRUCell(self.hidden_size)
            self.gru_cell_decoder = tf.nn.rnn_cell.GRUCell(2 * self.hidden_size)

    def gru_forward(self, embedded_words, gru_cell, reverse=False):
        outputs = []
        with tf.variable_scope("gru_forward") as scope:
            if reverse:
                embedded_words = tf.reverse(embedded_words, axis=[1])
                scope.reuse_variables()
            state = gru_cell.zero_state(self.batch_size, dtype=tf.float32)
            for time_step in range(self.sequence_length):
                if time_step > 0:
                    tf.get_variable_scope().reuse_variables()
                cell_output, state = gru_cell(embedded_words[:, time_step, :], state)
